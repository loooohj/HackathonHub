...
<body>
  ...
  <img src="IMAGES/<?=$h->img?>" class="card-img-top img-thumbnail rounded mx-auto d-block " style="height: 550px;width:80%;" alt="<?=$h->name_hackathon?>">
  ...
  <a class="nav-link sign border border-white rounded py-5 text-center fs-1" href="index.php?controller=participation&action=participate&id_hackathon=<?=$h->id_hackathon?>&max=<?=$h->max_num_members?>&min=<?=$h->min_num_members?>">Participate</a>  
</div>
